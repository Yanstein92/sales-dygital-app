...
export const Dashboard: React.FC<{
  ...
  const [filterFacture, setFilterFacture] = useState('Tous'); // 'Tous', 'Facturé', 'Non facturé'
  ...
  let processedSales = sales.filter(s => {
    ...
    if (filterFacture === 'Non facturé' && s.factureStatus === 'facture') return false;
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div 
        ...
        className={`bg-white p-4 rounded-xl shadow-sm border-2 transition-all duration-200 relative ${isDragging ? 'border-purple-500 border-dashed bg-purple-50 scale-[1.01]' : 'border-slate-200'}`}
      >
        {isDragging && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-purple-500/10 rounded-xl border-2 border-purple-500 border-dashed backdrop-blur-[1px] pointer-events-none">
            <div className="bg-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 text-purple-700 font-black animate-bounce pointer-events-none">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="relative w-full md:w-96">
              ...
              <input 
                type="text" 
                className="block w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-medium transition-colors" 
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center gap-4 border-t border-slate-100 pt-4">
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
              <div className="flex items-center space-x-2 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                ...
                <select className="bg-transparent border-none text-sm focus:ring-0 text-slate-700 outline-none cursor-pointer font-bold" value={filterCommercial} onChange={(e) => setFilterCommercial(e.target.value)}>
                  {commerciaux.map(c => <option key={c} value={c}>{c === 'Tous' ? 'Tous les commerciaux' : c}</option>)}
                </select>
              </div>
              <div className="flex items-center space-x-2 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
              ...
              {/* NEW FILTERS */}
              <select className="bg-slate-100 px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none font-bold text-slate-700 cursor-pointer" value={filterFacture} onChange={(e) => setFilterFacture(e.target.value)}>
              ...
              <select className="bg-slate-100 px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none font-bold text-slate-700 cursor-pointer" value={filterReleaseStatus} onChange={(e) => setFilterReleaseStatus(e.target.value)}>
            ...
            <div className="flex items-center gap-2 w-full lg:w-auto flex-wrap lg:flex-nowrap">
              ...
              <button onClick={() => handleAddSaleCheck(() => pdfInputRef.current?.click())} className={`flex flex-none items-center justify-center p-2.5 rounded-lg transition-all shadow-md transform hover:-translate-y-0.5 border bg-white text-purple-600 border-purple-200 hover:bg-purple-50`} title="Scanner un PDF">
                <UploadCloud size={20} />
              </button>
              <button onClick={() => handleAddSaleCheck(onManualEntry)} className="flex flex-1 lg:flex-none items-center justify-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg transition-all text-sm font-black shadow-md transform hover:-translate-y-0.5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600 min-w-[1000px]">
             <thead className="bg-slate-50 border-b border-slate-200 text-slate-700 uppercase text-xs font-bold tracking-wider">
              <tr>
                <th className="px-4 py-4 cursor-pointer hover:bg-slate-200 transition-colors group select-none" onClick={() => handleSort('bdcNumber')}>
                  <div className="flex items-center gap-2">N° BDC <SortIcon columnKey="bdcNumber" /></div>
                </th>
                <th className="px-4 py-4 cursor-pointer hover:bg-slate-200 transition-colors group select-none" onClick={() => handleSort('client')}>
                  <div className="flex items-center gap-2">Client & Contact <SortIcon columnKey="client" /></div>
                </th>
                <th className="px-4 py-4 cursor-pointer hover:bg-slate-200 transition-colors group select-none" onClick={() => handleSort('vehicule')}>
                  <div className="flex items-center gap-2">Véhicule & Infos <SortIcon columnKey="vehicule" /></div>
                </th>
                <th className="px-4 py-4 text-center cursor-pointer hover:bg-slate-200 transition-colors group select-none" onClick={() => handleSort('date')}>
                  <div className="flex items-center justify-center gap-2">Date & Délai <SortIcon columnKey="date" /></div>
                </th>
                <th className="px-4 py-4 text-right cursor-pointer hover:bg-slate-200 transition-colors group select-none" onClick={() => handleSort('reste')}>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-4 align-top w-24">
                      <div className="font-mono font-bold text-slate-800 bg-slate-200 px-2 py-1 rounded inline-block">#{sale.bdcNumber}</div>
                      <div className="mt-2">
                        <span className={`text-[10px] uppercase font-black px-1.5 py-0.5 rounded ${sale.company === 'KDB AUTO' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>{sale.company}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 align-top w-64">
                      ...
                      {sale.ref && <div className="text-xs font-bold text-slate-500 mt-0.5 flex items-center"><Hash size={10} className="mr-0.5 text-blue-500"/> Réf: {sale.ref}</div>}
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${sale.commercial === 'À assigner' ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-600'}`}>
                          {sale.commercial}
                        </span>
                    ...
                    <td className="px-4 py-4 align-top w-80">
                      <div className="font-bold text-slate-800">{sale.marque} {sale.modele} <span className="text-xs font-normal text-slate-500">({sale.color})</span></div>
                      <div className="mt-1.5 flex flex-col gap-1 items-start">
                        <span className="text-[11px] font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 flex items-center gap-1"><Car size={10}/> {sale.plaque || '-'}</span>
                        <div className="flex items-center gap-2">
                           <span className="text-[10px] text-slate-500 font-mono bg-slate-50 px-1.5 py-0.5 rounded border border-slate-100 break-all leading-tight">
                             {sale.vin || '-'}
                           </span>
                           {sale.releaseStatus && sale.releaseStatus !== 'non_sorti' && (
                             <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${sale.releaseStatus === 'programmee' ? 'bg-blue-100 text-blue-800' : 'bg-slate-800 text-white'}`}>
                               {sale.releaseStatus === 'programmee' ? 'Sortie Prog.' : sale.releaseStatus === 'sorti_tpd' ? 'Sorti TPD' : 'Sorti'}
                             </span>
                           )}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-center align-top w-32">
                        <div className="font-bold text-slate-700 mb-1.5">{new Date(sale.date).toLocaleDateString('fr-FR')}</div>
                        <div className="flex justify-center">
                          {isPaid ? <span className="text-xs font-bold text-slate-400 flex items-center gap-1"><CheckCircle2 size={12}/> Soldé</span> : isOverdue ? <span className="text-xs font-black text-red-600 flex items-center gap-1 bg-red-100 px-2 py-0.5 rounded"><Clock size={12}/> Retard {Math.abs(daysDiff)}j</span> : <span className="text-xs font-bold text-blue-600 flex items-center gap-1"><Clock size={12}/> Reste {daysDiff} j</span>}
                        </div>
                    </td>
                    <td className="px-4 py-4 text-right align-top w-36">
                      <span className={`px-3 py-1.5 rounded-md text-sm font-black shadow-sm inline-block min-w-[90px] text-center whitespace-nowrap ${isPaid ? 'bg-emerald-100 text-emerald-700' : isOverdue ? 'bg-red-600 text-white' : 'bg-amber-100 text-amber-800'}`}>{remaining.toLocaleString()} €</span>
                    </td>
                    <td className="px-4 py-4 text-center align-top w-20">
                      <button onClick={(e) => { e.stopPropagation(); onSelectSale(sale.id); }} className="text-blue-600 hover:text-white p-2.5 rounded-md hover:bg-blue-600 transition-colors border border-slate-200 hover:border-blue-600 shadow-sm bg-white"><CreditCard size={18} /></button>
                    </td>
                  </tr>
                );
              })}
